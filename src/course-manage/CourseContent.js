...
class CourseContent extends Component {
  ...
  courseToData(course) {
    ...
    const createTree = root => {
      ...
      root.hasChildNode.forEach(id =>
        root.children.push(course.data.filter(node => node._id === id)[0])
      );
  ...
  render() {
    ...
    return (
      ...
      >
        {graphData ? (
          <Graph graph={graphData} events={{ click: this.handleNodeClick }} />
        ) : (
          <h1>等待载入课程...</h1>
        )}
      </div>
...
const NoCurrentLesson = () => {
  return <div className="noCurrentLesson">暂无课程</div>;
};

export { NoCurrentLesson, CourseContent };
