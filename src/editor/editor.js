...
class Editor extends Component {
  ...
  handleCreate = () => {
    ...
    form.validateFields((err, values) => {
      ...
      updateOptions.projectName = values.title ? values.title : '';

      updateOptions.description = values.description ? values.description : '';
  ...
  canvasConstructor(canvas, dragBox, options, callback) {
    ...
    function initCanvas(canvas, dragBox) {
      let transform = createElement('div')
        .addClass('canvasTransform')
        .attr('id', 'canvasTransform');
      let start = createElement('div')
        .addClass(basicOptions.startClass)
        .attr('id', 'transStart')
        .text('Thing');
      let imgContainer = createElement('div')
        .addClass('canvasTransform')
        .attr('id', 'trans-img-cont');
      let svgContainer = createElement('div')
        .addClass('canvasTransform')
        .attr('id', 'trans-svg-cont');

      let remove = $('#deleteBtn').addClass(basicOptions.removeButtonClass);
      $(canvas)
        .append(transform.append(start, imgContainer, svgContainer))
        .css('overflow', 'hidden');
    ...
    function setEvent(canvas, dragBox, canv, info, callback) {
      let css_trans_origin = $(control.trans)
      ...
      fn.canvas.bind({
        mousedown: function(e) {
          ...
          if (
            ele.hasClass(basicOptions.imgBoxClass) ||
            ele.attr('id') === control.start.attr('id')
          ) {
            //开始按钮绑定的事件
            ...
            $(document).bind({
              ...
              mouseup: function(e) {
                ...
                let newPositionInfo = fn.canvas_input_abled(
                  control.imgCont,
                  ele
                );
        ...
        click: function(e) {
          ...
          if (ele.attr('class') === basicOptions.imgBoxClass) {
            ...
            ele.hasClass(basicOptions.circle) ||
            ele.parents('.' + basicOptions.circle)[0]
          ) {
            let svgobj;
      ...
      fn.removeButn.bind('click', function(e) {
        ...
        if ($(this).hasClass('remove-active')) {
          ...
          path_container.forEach(function(c) {
            if (c.from === ele[0]) {
              remove_ele(c, fn);
            } else if (c.to === ele[0]) {
          ...
          if (type === 'delElement') {
            ...
            $(ele[0].circle).remove();
            if (callback) {
              callback(type, { elmId: ele.attr('id'), butn: butnArray });
            }
            ...
            if (callback) {
              callback(type, { butn: [{ elmId: elmId, butnId: butnId }] });
            }
          } else {
    ...
    function deleteKnowledgeUnit(kUnit) {
      ...
      if (kUnit.hasParentNode.length !== 0) {
        for (let index in kUnit.hasParentNode) {
          tempUnit = _this.getKnowledgeObjectById(
            kUnit.hasParentNode[index]._id
          );
          _this.updateKnowledgeUnit(
            unlink(tempUnit, kUnit._id, 'hasChildNode')
          );
        }
      }
      if (kUnit.hasChildNode.length !== 0) {
        for (let index in kUnit.hasChildNode) {
          tempUnit = _this.getKnowledgeObjectById(
            kUnit.hasChildNode[index]._id
          );
          _this.updateKnowledgeUnit(
            unlink(tempUnit, kUnit._id, 'hasParentNode')
          );
        }
      }
      for (let index in KnowledgeObjects) {
        if (KnowledgeObjects[index]._id === kUnit._id) {
          KnowledgeObjects.splice(parseInt(index, 10), 1);
          _this.setState({
    ...
    func.prototype.init = function() {
      ...
      info.forEach(function(elm) {
        let div = createNewElement(
          fun,
          elm._id,
          elm.thumbnailUrl,
          elm.videoUrl
    ...
    func.prototype.move_canvas_ele = function(e, ele, click_position_on_img) {
      ...
      ele.css({
        left:
          (mouse_position.left -
            trans_position.left -
            click_position_on_img.left) /
          scale,
        top:
          (mouse_position.top -
            trans_position.top -
            click_position_on_img.top) /
          scale
      });
      for (var i in path_container) {
        if (
          ele[0] === path_container[i].from ||
          ele[0] === path_container[i].to
        ) {
          path_move_with_img(
            $(path_container[i].from),
            $(path_container[i].to),
            $(path_container[i].path)
          );
        }
      }
    ...
    func.prototype.move_canvas = function(
      e,
      old_client_position,
      trans_position,
      css_trans_origin,
      ...
      this.trans.offset(function(index, oldoffset) {
        return {
          left:
            trans_position.left +
            mouse_position.left -
            old_client_position.left,
          top: trans_position.top + mouse_position.top - old_client_position.top
        };
      });
      trans_offset = get_ele_position_of_window(this.trans);
      originX =
        (parseInt(css_trans_origin[0], 10) +
          css_trans_offset.left -
          trans_offset.left) /
        scale;
      originY =
        (parseInt(css_trans_origin[1], 10) +
          css_trans_offset.top -
          trans_offset.top) /
        scale;
    ...
    func.prototype.path_moving = function(e, svgobj, ele) {
      ...
      var deltaX = (e.clientX - parseInt(start_point.left, 10)) / scale;
      var deltaY = (e.clientY - parseInt(start_point.top, 10)) / scale;
      makeSvg(
        deltaX,
        deltaY,
        svgobj.svg,
        svgobj.svg_ori_position.left,
        svgobj.svg_ori_position.top
      );
      makePath(
        deltaX,
        deltaY,
        svgobj.svg.children('.path'),
        svgobj.svg.children('.arrow')
      );
    };
    ...
    func.prototype.path_highLight = function(svg) {
      path_highLight_ended();
      svg.children('path').forEach(function(index) {
        svg
          .children('path')
        index.setAttribute('stroke', basicOptions.pathHighLight);
      });
      svg
        .children('.arrow')[0]
        .setAttribute('fill', basicOptions.pathHighLight);
    ...
    func.prototype.canvas_input_abled = function(_this, ele) {
      ...
      if (ele) {
        return {
          _id: ele.attr('id'),
          position: {
            x: parseInt(ele.css('left').match(/[^px]+/),10),
            y: parseInt(ele.css('top').match(/[^px]+/),10)
          }
        };
      }
    };
      ...
      eventobj.hover(
        ...
        function(e) {
          if (_this.getTarget(e).parents('.' + _this.circleCover)[0]) {
            _this
              .getTarget(e)
              .parents('.' + _this.circleCover)
              .children('svg')[0]
              .setAttribute('display', 'none');
            if (!path_from)
              _this.svgContainer.find('.path').css('pointer-events', 'auto');
          }
        }
      );
    };
    ...
    function path_highLight_ended() {
      path_container.forEach(function(c) {
        if (c.path) {
          $(c.path)
            .children('path')[0]
            .setAttribute('stroke', basicOptions.pathBottomColor);
          $(c.path)
            .children('path')[1]
            .setAttribute('stroke', basicOptions.pathAboveColor);
          $(c.path)
            .children('.arrow')[0]
            .setAttribute('stroke', basicOptions.pathAboveColor);
          $(c.path)
            .children('.arrow')[0]
            .setAttribute('fill', basicOptions.pathAboveColor);
    ...
    function get_img_size() {
      return [
        control.dragBox.children('.' + basicOptions.imgBoxClass).innerWidth() /
          2,
        control.dragBox.children('.' + basicOptions.imgBoxClass).innerHeight() /
          2
      ];
    }
    ...
    function addDragBoxGhost(e, ele) {
      ...
      dragBoxGhost.addClass('canvas-api-floatImg').css({
    ...
    function createNewElement(fun, id, imgUrl, videoUrl) {
      ...
      newBox = createElement('div')
        .addClass(basicOptions.imgBoxClass)
        .css({
          position: 'absolute',
          margin: '0'
        })
      ...
      img = !imgUrl
        ? createElement('img').attr(
            'src',
            'http://localhost:3000/defaultImg.jpg'
          )
        : createElement('img').attr('src', imgUrl);
      newBox.append(img);
      if (videoUrl) {
    ...
    function create_circle(ele, fun) {
      let cover = createElement('div').addClass(
        'img-box-circle-cover ' + fun.circleCover
      );
    ...
    function layout_title(fun) {
      ...
      let imgSize = getELemSize(control.dragBox);
      // let imgSize = get_img_size();
      let node = fun.imgContainer.children('.' + basicOptions.imgBoxClass)
        .length;
      input
        .attr({
          type: 'text',
    ...
    function circle_comewith_ele(ele, circl) {
      ...
      circle.css({
        left:
          parseInt(ele.css('left'), 10) + ele.innerWidth() - circle.width() / 2,
        top:
          parseInt(ele.css('top'), 10) +
          ele.innerHeight() / 2 -
          circle.height() / 2
      });
    }
    ...
    function clear_path(_this) {
      if (_this.svgContainer.find('.path-active'))
        _this.svgContainer.find('.path-active').remove();
    ...
    function set_path_cssAndAttr(svg, path, path_back, arrow) {
      ...
      arrow[0].style.cssText = path_back[0].style.cssText = path[0].style.cssText =
    ...
    function get_path_from_position(ele) {
      return {
        left: parseInt(ele.css('left'), 10) + ele.innerWidth(),
        top: parseInt(ele.css('top'), 10) - (90 - ele.innerHeight()) / 2
      };
    }
    ...
    function path_move_with_img(from, to, path) {
      ...
      makeSvg(
        ...
        delta.deltaY,
        path,
        path_from_position.left,
        path_from_position.top
      );
      makePath(
        delta.deltaX,
        delta.deltaY,
    ...
    function get_deltaXY(from, to) {
      let deltaX =
        parseInt(to.css('left'), 10) -
        parseInt(from.css('left'), 10) -
        from.width();
      let deltaY =
        parseInt(to.css('top'), 10) +
        to.height() / 2 -
        parseInt(from.css('top'), 10) -
        from.height() / 2;
      return {
        deltaX: deltaX,
        deltaY: deltaY
      ...
      if (deltaX > 42) {
        ...
        if (deltaX > 29) {
          if (deltaY < -24 && deltaY > -45) {
            path.attr(
              'd',
              'M 0 45 L 15 45 A 6 6,0 0 0,21 39 L 21 ' +
                (66 - deltaX / 2 + deltaY / 2) +
                ' A ' +
                (42 - deltaX) / 2 +
                ' ' +
                (42 - deltaX) / 2 +
            ...
            path.attr(
              'd',
              'M 0 45 L 15 45 A 6 6,0 0 1,21 51 L 21 ' +
                (24 + deltaY / 2 + deltaX / 2) +
                ' A ' +
                (42 - deltaX) / 2 +
                ' ' +
            ...
            path.attr(
              'd',
              'M 0 0 L 15 0 A 6 6,0 0 1,21 6 L 21 ' +
                (deltaY / 2 + deltaX / 2 - 21) +
                ' A ' +
                (21 - deltaX / 2) +
                ' ' +
                (21 - deltaX / 2) +
          ...
          if (deltaY < -24 && deltaY > -45) {
            ...
            path.attr(
              ...
              'M ' +
                ...
                ' A 6 6,0 0 0,0 ' +
                (96 - deltaY / 2) +
                ' L 0 86 A 6 6,0 0 0,6 90 L 21 90'
            );
            arrow.attr(
              'd',
              'M ' +
  ...
  saveProject() {
    ...
    _this.request(
      '/saveProjectData',
      { username: _this.state.username, projectData: data },
      function() {
  ...
  updateKnowledgeUnit(kUnit) {
    ...
    for (let index in KnowledgeObjects) {
      if (KnowledgeObjects[index]._id === kUnit._id) {
        KnowledgeObjects.splice(parseInt(index, 10), 1, kUnit);
        this.setState({
          knowledgeUnitList: KnowledgeObjects
        });
      }
  ...
  static_path = (ele, path_from) => {
    ...
    this.state.path_container.push(
      new path_container_construct(path_from, tagSvg, ele)
    );
  };
  ...
  render() {
    ...
    return (
      <div id="editorArea">
        <div id="toolBar">
          <div id="dragBox" className="dragBox">
            知识单元
          </div>
          <Button
          ...
          <Button id="deleteBtn" className="editBtn editorBtn ant-btn-primary">
            删除
          </Button>
    ...
    return (
      ...
      >
        <Form layout="vertical">
          <FormItem label="课程名称">
            {getFieldDecorator('title')(<Input placeholder={projectName} />)}
          </FormItem>
          <FormItem label="课程描述">
          ...
          <FormItem label="课程缩略图">
            {getFieldDecorator('thumbnailUrl')(
              <Select
                ...
                id="material"
              >
                <OptGroup label="图片">{children}</OptGroup>
              </Select>
            )}
          </FormItem>
        </Form>
